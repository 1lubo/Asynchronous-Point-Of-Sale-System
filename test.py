...
async def test():
    ...
    while user_entry != 'no':
        ...
        print(f'ordered items {order_summary}')
        ...
        burgers = list(
            filter(lambda x: x['category'] == 'Burgers', final_order))
        ...
        sides.sort(key=lambda item: item.get('price'), reverse=True)
        drinks.sort(key=lambda item: item.get('price'), reverse=True)

        combo = []
        total_price = []
        while len(burgers) > 0 and len(sides) > 0 and len(drinks) > 0:
            c_i = [burgers[0], sides[0], drinks[0]]
            combo.append(c_i)
            burgers.pop(0)
            sides.pop(0)
            drinks.pop(0)
